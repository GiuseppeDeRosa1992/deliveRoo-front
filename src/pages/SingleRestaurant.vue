    ...
    <div class="mt-5">
        <div class="row m-0">
            ...
            <div class="col-12 col-md-5 col-lg-4 p-0 px-2">
                ...
                <div class="card cart-card my-position mb-5">
                    <div class="card-body">
                        ...
                        <p v-if="!Store.recordCart.length > 0" class="card-text">Aggiungi piatti al carrello per
                            visualizzare qui.</p>
                        <div v-if="Store.recordCart.length > 0" class="cart-items mb-0">
                            <div class="cart-list ps-0">
                                <h3 class="card-title m-0 py-2">Stai ordinando presso: {{
                                    Store.recordCart[0].restaurant.name }}</h3>
                                    ...
                                    <template v-for="(product, i) in Store.recordCart[0].products">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between pb-2">
                                            <div class="quantity-controls ms-0">
                                                <button @click="Function.decrementQuantity(product, i)"
                                                    class="rounded border-0 text-white btn-quantity ms-0">
                                                    -
                                                </button>
                                                <span>{{ product.quantity }}</span>
                                                <button @click="Function.incrementQuantity(product)"
                                                    class="rounded border-0 text-white btn-quantity">
                                                    +
                                                </button>
                                            </div>
                                            <button @click="Function.deleteProduct(i)"
                        ...
                        <div v-if="Store.recordCart.length > 0"
                            class="cart-totals border-top border-2 border-dark pt-2">
                        ...
                        <button class="btn btn-success" :disabled="!Store.recordCart.length > 0">Procedi
                            all'ordine</button>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="confirmModal" tabindex="-1" aria-labelledby="confirmModalLabel"
                aria-hidden="true" v-if="Store.recordCart.length > 0">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h4 class="modal-title" id="confirmModalLabel">Stai Già ordinando da: {{
                                Store.recordCart[0].restaurant.name }}</h4>
                        ...
                        <div class="modal-footer d-flex flex-column align-items-center">
                            <a
                                :href="'http://localhost:5174/restaurant/' + Store.recordCart[0].restaurant.slug + '/menu'">
                                <button type=" button" class="btn btn-primary" data-bs-dismiss="modal">
                                    Completa l'ordine
                                </button>
                            </a>
                            <button type="button" class="btn btn-secondary" @click="Function.deleteCart()"
