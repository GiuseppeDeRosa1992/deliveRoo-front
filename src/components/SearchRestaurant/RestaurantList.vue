...
<template>
	<template v-if="!Store.loader">
		<div class="container" v-show="Store.listRestaurants">
			...
			<h3 v-if="Store.listRestaurants.length > 0" class="m-0 text-center mb-5">Scegli il tuo ristorante</h3>
			<h3 v-else class="m-0 text-center mb-5">Nessun ristorante trovato per le categorie selezionate</h3>
							...
							<div class="card text-center h-100 my-shadow">
								<div class="card-body p-0 px-2 pb-2 d-flex justify-content-between flex-column">
									<div class="position-relative">
										...
										<img v-else="" :src="restaurant.image" class="rounded rounded-circle position-absolute mt-4" alt="..." />
